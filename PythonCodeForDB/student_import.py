...
def generate_random_student(num_names=1):
...
df = pd.DataFrame(columns=['First Name', 'Last Name', 'Student Number', 'Email'])

# Generate 100 random students
student_names = generate_random_student(100)
for name in student_names:
    ...
    val = (name[2], name[0], name[1], f"{name[0]}.{name[1]}@student.kuleuven.be", role, hash(password), 1, token)
